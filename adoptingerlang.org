...
#+attr_latex: :environment bclogo :options [logo=\bcinfo, couleurBarre=orange, noborder=true, couleur=white]{Information}
#+begin_notice
You don't have to start everything to get an Erlang shell from your release. Every release built with =rebar3= comes with a boot script called =start_clean= which only starts the =kernel= and =stdlib= applications. This can be run with the command =console_clean= and can be useful for debugging purposes.
#+end_notice
