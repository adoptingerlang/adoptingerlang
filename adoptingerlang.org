...
#+HUGO_PAIRED_SHORTCODES: codecaption
...
Flycheck comes with =rebar3= support and can automatically detect a rebar3 project, so all that is needed is the =flycheck= package. =use-package=  The packages =flycheck-inline= and =flycheck-pos-tip= improve usability but are optional:
...
#+attr_shortcode: lang="elisp" title="Hydra Flycheck"
#+BEGIN_codecaption elisp
(use-package hydra
  :defer 2
  :bind ("C-c f" . hydra-flycheck/body))
