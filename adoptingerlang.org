...
Each container spec in the =Pod= spec can include resource requests and limits for memory and CPU.
...
CPU settings don't actually limit the number of cores available to the VM.

Request only taken into account when scheduling what node a pod goes on

Limit is what is translated to cgroup quota and period for cpu.
