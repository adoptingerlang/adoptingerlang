...
There are a few exceptions to these. For example, dependencies are shared for the project, so while each top-level app can specify its own dependencies, they can't have wildly different versions. At the opposite, directories such as =priv/= are meant for a single app's private files and cannot be shared.

Another small subtlety is around hooks; some hooks can be defined both for a single OTP application and for a whole project. For example, a =compile= hook defined at the top level will run before and after _all_ applications are built, and the same hook defined for a single application in =apps/= will run only before and after that one application is compiled.
...
- Use [[TODO: CHANGE LINK][templates]] stored in some general [[TODO: CHANGE LINK][plugin]] that your teammates will install globally to automate the layout and specification of services, web APIs, CI configurations, and so on
