...
The next stage uses the =builder= image as it's base:
...
RUN --mount=type=cache,target=/root/.cache/rebar3 rebar3 dialyzer \
    --plt-location $HOME/.cache/rebar3 --plt-prefix deps --base-plt-prefix otp

ENTRYPOINT ["rebar3"]

CMD ["dialyzer", "--plt-location", "$HOME/.cache/rebar3", "--plt-prefix",
