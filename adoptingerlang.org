...
#+NAME: url_iodata
#+BEGIN_SRC erlang
Directory = <<"/Users/ferd/code/self/ReVault">>,
FileName = "src/my_module",
[Directory, $/, FileName, ".erl"]
#+END_SRC

The final data structure here looks like =[<<"/Users/ferd/code/self/ReVault">>,47,"src/my_module", ".erl"]= which is a mixed list containing binary subsections, literal codepoints, strings, or other IoData structures. But the VM mechanisms all support handling it as if it were a flat binary string: The IO systems (both network and disk access), and the modules named in the previous section all seamlessly handle this string as =/Users/ferd/code/self/ReVault/src/my_module.erl= with full Unicode support.
