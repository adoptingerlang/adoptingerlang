...
In Kubernetes containers are part of a =Pod=. Each =Pod= has 1 or more containers, and optionally =init-containers= which run once before the containers that make up the running =Pod= are started. For an application a higher level abstraction, a =Deployment=, is used so individual =Pods= don't have to be created for scaling or deploying. A =Deployment= is a declarative way to create and update how many =Pods= of our application are to be run and how they are to be deployed.
...
#+BEGIN_SRC shell
$ kubectl apply -k deployment/dev
#+END_SRC
