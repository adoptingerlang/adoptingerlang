...
#+attr_latex: :environment bclogo :options [logo=\bcinfo, couleurBarre=orange, noborder=true, couleur=white]{Information}
#+begin_notice
Security notes: Clear cache to update packages.
#+end_notice
...
It would be tedious to have to manually build the images needed and publish to the registry. Instead including image building as part of the Continuous Integration process is common. Usually this is restricted to only occur on a merge to master or a new tag being created, but it can also be useful to always build the images, even for branches, for testing purposes. In this section we will cover a couple options for automating this process. However, there is no shortage of solutions and chances are if you are already using a particular CI tool it will be capable of doing the same with a similar approach.
...
Kubernetes is a container orchestration system. When our backend contains many distinct services that need to be kept running, cross communicating and scaling an orchestration system becomes essential. And with the growing popularity of containers Kubernetes for orchestration has grown as well.
...
In order to develop and test the Kubernetes deploy we will need to run Kubernetes. Kubernetes locally for testing purposes and briefly discuss options for production deployments.

**** Locally

- [[https://github.com/kubernetes/minikube][minikube]]: The longest existing option that is now part of the Kubernetes project. minikube offers support for various hypervisors. It has an option to run without creating a new virtual machine but it suggests still doing so within a Linux VM and not directly on your machine.
...
Since, as we saw in the Releases chapter, runtime configuration is done through environment variable substitution in =vm.args.src= and =sys.config.src= we to insert those variable to the environment of the container. Each container in a Kubernetes Pod can have a field =env= declaring a set of environment variables:
...
Now all the variables defined in the ConfigMap will be added to the container without having to individually specify each one.
