...
Docker images contain multiple layers that when merged at runtime make up the filesystem of the container. The layers are created by docker running commands found in a =Dockerfile=, each command creates a new layer.

Because, unlike a virtual machine (VM) image, the docker image does not need to contain the Linux kernel, images can often be barely larger than your original program and the use of layers allows for images to save even more space when multiple images require some of the same layers.
...
The small size and ability to run like a regular Linux process makes for quicker start time and less resource consumption than using a traditional VM for isolation. Having little overhead means that the advantages of isolation when packaging and running a program can be standard practice instead of the burden it would be to have to run a VM per program.
