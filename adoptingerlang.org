...
- All dependencies can define their own dependencies, and rebar3 will recognize this and put them within your own project's =_build= directory as well
...
- Rebar3 supports composable per-project [[][_profiles_] that let you segment or combine configuration settings, including which dependencies are to be used only for tests, or in specific environment (say on a specific target OS)
- Dependencies are always built with the =prod= profile applied, and once built will not be recompiled unless they are upgraded, even though the tool will always make sure they are there, match the lock and are compiled
...
Since dependencies are all project-local, they have to be declared in the =rebar.config= file of your project. This will let Rebar3 know it needs to fetch them, build them, and generally make them available to your project. All dependencies must be individual OTP applications, so that they can be versioned and handled independently from each other.
