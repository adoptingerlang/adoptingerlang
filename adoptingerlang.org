...
- As part of that declarative approach, commands in Rebar3 are defined with dependency sequences. For example, the =rebar3 compile= task relies on =rebar3 get-deps=, and will run it for you. The command =rebar3 tar= will implicitly call a sequence that includes =get-deps -> compile -> release -> tar= for you.
- Rebar3 therefore knows that to compile your own project, its dependencies are needed; it will gladly fetch dependencies and ensure they're up to date for you every time you ask for it to compile your project
...
That's a lot of properties, but we find that it's useful to internalize these before getting in too deep with dependencies. If you operate under assumptions that Rebar3 works like Javascript's =npm=, Elixir's =mix=, or Java's =maven=, you might find some behaviours confusing.
...
- patching dependencies (checkouts)
- upgrading
