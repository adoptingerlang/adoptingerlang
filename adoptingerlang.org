...
This chapter will give some low level details about how a release is constructed, but the main purpose is to use the [[https://github.com/adoptingerlang/service_discovery][service_discovery]] project to show how to use =rebar3= for building releases suited for local development, then a production ready release and finally how to use the tools a release built with =rebar3= comes with to configure and operate a release. And ultimately show that the advantages from the flexibility of booting concurrently running applications do not have to come with the cost of esoteric operations.
