...
"connection refused" error in response to a request during while booting and to
do this =startupProbe= should be used instead of =readinessProbe=. Unlike
=readinessProbe= the =startProbe= will only be checked once until it passes, not
periodically for the life of the =Pod=. If the startup doesn't take many seconds
or minutes and the clients that may send it requests while down are not
negatively effected by simply having to do retries until they connect to a ready
node it isn't necessarily needed. To do this the =service_discovery= project uses
a =startupProbe=:
...
Because the boot order of the Release ensure the database is connected to and
DNS is up before the Elli server is started he probe only needs to be check that
the HTTP server is up and is written as:
