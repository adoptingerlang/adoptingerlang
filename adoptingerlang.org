...
Docker images contain multiple layers that when merged at runtime make up the filesystem of the container. Docker creates the layers by running commands found in a =Dockerfile=. Each command creates a new layer.

Unlike a virtual machine (VM) image, a Docker image does not need to contain the Linux kernel, and can often be nearly as small as your original program. Using layers allows images to save even more space when multiple of them depend on a shared layer.

No kernel is required because containers are run through functionality built-in to Linux for processes in isolated namespaces and controlling resource usage of a group of processes.

The small size and ability to run like a regular Linux process makes for quicker start times and less resource consumption than using a traditional VM for isolation. Having little overhead means that the advantages of isolation when packaging and running a program can be standard practice instead of the burden it would be to have to run a VM per program.

Namespace isolation means not having to worry about which libraries (OpenSSL for instance) are installed, if any, on the target machine, what ports are available or global configuration effecting separate instances of the same program.
...
#+attr_latex: :environment bclogo :options [logo=\bcinfo, couleurBarre=orange, noborder=true, couleur=white]{Information}
#+begin_notice
You might notice we are not using the =latest= tag at all when using Docker. This tag is commonly misunderstood and misused. It is assigned to the last image used without a specific tag, it is not the latest created image. It should not be used, unless you really don't care what version of an image will be used.
+end_alert
