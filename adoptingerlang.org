...
Some advantages of filesystem and network isolation are not having to worry about which libraries (OpenSSL for instance) are installed on the target machine, if a particular port is already bound, if an Erlang node with the same name is already running or global configuration effecting separate instances of the same program.
