...
First, a =RUN= command installs the necessary dependencies, SSH and git. Then, =ssh-keyscan= is used to download the current public key for Github and add it to =known_hosts=. This ensures that a response is not needed to the SSH prompt asking if you accept the host's public key, which it would ask since in the image build this is the first request being made to the host and would not yet be a known host.

Next, the git config setting set in the same =RUN= ensures that even if in the =rebar.config= the git url is using =https= it will instead use SSH. If the private repos are not on Github this url replacement has to be changed for the appropriate location.
