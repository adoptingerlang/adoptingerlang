...
Now this is all well and good but the layer with the built dependencies also contains the Hex package cache =rebar3= creates under =~/.cache/rebar3/hex=. Any change to those =rebar.config= or =rebar.lock= will result in all the packages having to not only be rebuilt but also be downloaded again. The next layers that copy in the whole project and compile it are also larger than they need to be. We don't need to keep the whole project source in a layer, we are only interested in the build artifacts.
...
Next, unlike the previous instructions for building the rest of the project the instruction =COPY . .= has been removed. Instead, a mount of type =bind= (the default) is used with a =target= of =.=. Since it is a bind mount, as opposed to the =cache= mount, Docker will mount from the host into the container, giving us the same result as =COPY . .= but without actually copying in the files into the layer. This mount is read-only, meaning the build will error out if anything is written to =/app/src=, and that is why the =rebar3= base directory has been configured to =/app/_build=.
...
The =builder= stage starts with the base image =erlang:22-alpine=. =as builder= names the stage so we can use it as a base image to =FROM= in later stages and as a target to =docker build=:

#+BEGIN_SRC shell
$ CHKSUM=$(cat rebar.config rebar.lock | cksum | awk '{print $1}')
$ docker build --target builder -t ${PROJECT}:builder-${CHKSUM} .
#+END_SRC

Running =docker build= with =--target builder= and =-t service_discovery:builder-$CHKSUM= builds the stage and tags with the given name.

The image with the compiled dependencies and project is now able to be used as a base for images that run rebar3 commands which would otherwise fetch and compile dependencies.
...
Now to build and tag the =release= target using the builder image and, if it exists, a previous build of the image that has the same tag (because have been an image built for the same git commit previously) as a cache:

#+BEGIN_SRC shell
$ docker build --cache-from=${PROJECT}:builder-${CHKSUM} --cache-from=${PROJECT}:releaser-${CHKSUM} --target releaser -t ${PROJECT}:releaser-${CHKSUM} .
#+END_SRC
...
#+BEGIN_SRC shell
$ docker build --cache-from=${PROJECT}:builder-$CHKSUM --target plt -t ${PROJECT}:plt-${CHKSUM} .
#+END_SRC

To run dialyzer on the current code use a volume to mount the current directory to =/src=:

#+BEGIN_SRC dockerfile
$ docker run -v $(pwd):/app/src ${PROJECT}:plt-${CHKSUM}
===> Verifying dependencies...
===> Compiling service_discovery_storage
===> Compiling service_discovery
===> Compiling service_discovery_http
...
===> Updating plt...
===> Resolving files...
===> Checking 296 files in "/app/_build/default/rebar3_22.0.7_plt"...
===> Doing success typing analysis...
===> Resolving files...
===> Analyzing 19 files with "/app/_build/default/rebar3_22.0.7_plt"...
#+END_SRC
...
#+BEGIN_SRC shell
$ docker build --cache-from=${PROJECT}:releaser-${CHKSUM} --cache-from=${PROJECT}:builder-${CHKSUM} --target runner -t ${PROJECT}:${GIT_REF} .
#+END_SRC
...
Using Kaniko in Cloud Build offers layer caching based on the contents of the Dockerfile line and those preceding it. Then, future builds check the layer cache in the image registry for matches before building any layer. However, the Buildkit features we used in the Dockerfile are not available in Kaniko, so a separate Dockerfile, =ci/Dockerfile.cb=, is used:
