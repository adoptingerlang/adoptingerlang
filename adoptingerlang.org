...
- [[https://cloud.google.com/kubernetes-engine/][Google Kubernetes Engine]]: Google Cloud was used for writing this chapter, when not using microk8s, because it offers [[https://cloud.google.com/free/][$300 of credit when signing up as a new user]]
...
The =fieldRef= =status.podIP= will return the current Pod's IP when it is created and the =resourceFieldRef= of =limits.cpu= with a =divisor= of =1= will return the CPU limit in a unit of CPU cores.
...
The volume =migrations= exists only in memory and is used to copy the current SQL migration files from the release image to a shared directory that the =flyway= container will use to run validation. The =emptyDir= type of volume with =medium= =memory= is used because we are not using this volume to persist any data, only to share between two =Init Contianers=. If =medium: Memory= was not included the =emptyDir= is created on the host file system but it is still empty initially for each new =Pod= and is removed when the =Pod= is deleted.

**** Readiness and Liveness Probes

Kubernetes has multiple configurable probes to know when to restart a container and when a container is in a state it can accept traffic. The main two are the liveness and readiness probes, which handle container restart and whether traffic can be sent respectively. Additionally, a startup probe can be used to delay when the liveness and readiness probe start being checked.

Probes are defined as either a command to execute in the container, a HTTP =GET= request to make to a port and path or an attempt to open a TCP connection to a specific port.

A =livenessProbe= is not necessarily needed. If the process in the container crashes or stop for any reason it will be restarted. The probe ensures that if our node does not crash but for some reason has deadlocked and can not respond to requests it will also be restarted.
...
Kubernetes =Deployment= resources have a configurable [[https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#strategy][strategy]] for what happens when a new version of an image is deployed. The =service_discovery= =Deployment= uses a rolling update strategy, the alternative strategy is =Recreate= which means Kubernetes first terminates all =Pods= in the =Deployment= and then starts the new =Pods=. For the =RollingUpdate= strategy the Kubernetes =Deployment= controller will bring up new =Pods= and then terminate the old. There are two configuration variables to tell the controller how many =Pods= above the desired amount are allowed (=maxSurge=) and how many are allowed to be unavailable during the deploy (=maxUnavailable=).
...
<p>There were a number of issues with Helm aside from the templating that we won't go into here, except to say that they are being worked out in the next major release, <a href="https://v3.helm.sh/">Helm v3</a>. As of this writing in November 2019 has release candidates available. So I would still suggest taking a look at the latest offering from Helm.</p>
#+end_admonition
...
[[https://tilt.dev/][Tilt]] is a tool for deploying and updating Docker images and Kubernetes deploys locally for development purposes. The easiest way to get started with Tilt is using a registry within the local Kubernetes cluster. For =microk8s= a registry can be enabled easily, we will need DNS as well so enable that at this time as well:
...
=allow_k8s_contexts= restricts the Kubernetes context that Tilt will run against to =microk8s=, this is to protect the user from accidentally deploying their local development resources and images to an external Kubernetes cluster, like production! =default_registry= sets where to push Docker images built by Tilt, in this case the registry enabled in =microk8s= is used.
...
The target is =dev_release= because we want the ability to do live updates simply by recompiling and restarting inside the running image:
...
The =live_update= instructions will run in the container any time a file changes. Since a =dev_mode= release is used, the Rebar3 profile =tilt= keeps the default =dev_mode= of =true=, the update commands simply need to sync the =apps= directory to the running container, run =compile= and restart the release.

Lastly in the =Tiltfile= the Kubernetes resources to deploy are configured and a watch is set to rerun the =Tiltfile= if any of the kustomize files changes.
...
This patch changes the image name in the =Job= resource to =service_discovery_sql=, the same name as the first =custom_build= image in the =Tiltfile=. Tilt will update the image with the latest tag, which it sets as =$EXPECTED_REF= in the environment of the Docker build command in =custom_build=, and rerun the =Job=. This way while =service_discovery= is running in the local Kubernetes if a new migration is added it will automatically be picked up and run on the database, keeping our development cluster in sync with our local development.
